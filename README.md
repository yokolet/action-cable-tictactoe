...
1. Initial screen\
Initial screen is like this. Only one clickable is the "Register" button.\
...
single quote, hyphen, and underscore are available to use. The maximum number of players is limited to 20.\
...
Open another browser or private window to visit the top page. Before registration, people can see registered players
and created boards.\
When the second player registers, the name instantly appears on the first player's screen. If the second player clicks
the existing board, that person takes O (the second player). Then, the game starts.\
...
Players will be deleted when "Unregister" button is clicked or close a tag/window.
